/**
 * VideoDownloadTracker - Global Video Download Tracking Utility
 *
 * OVERVIEW:
...
export interface VideoDownloadTrackerConfig {
...
export class VideoDownloadTracker {
    private totalSize = 0;
    ...
    private config: VideoDownloadTrackerConfig;
    private currentVideoUrl = '';
    private currentVideoElement: HTMLVideoElement | null = null;

    constructor(config: VideoDownloadTrackerConfig = {}) {
        this.config = {
     ...
     */
    private async updateProgressEstimate(
        ranges: BufferedRange[]
    ): Promise<void> {
    ...
    private timeToByteRange(
        ...
        const start = Math.max(
            0,
            Math.floor((startTime / duration) * totalSize)
        );
        const end = Math.min(
            totalSize - 1,
     ...
     */
    private async calculateTotalBytes(
        ranges: BufferedRange[]
    ): Promise<number> {
        let totalBytes = 0;
    ...
    private updateDownloadMetrics(totalBytes: number): void {
        const deltaBytes = Math.max(
            0,
            totalBytes - this.submittedBytesBaseline
        );
